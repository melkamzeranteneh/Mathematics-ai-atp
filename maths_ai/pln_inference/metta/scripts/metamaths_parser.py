...
def get_existing_names(output_path):
    """Read rule names already present in the output file."""
    import re, os
    if not os.path.exists(output_path):
        return set()
    with open(output_path, 'r', encoding='utf-8') as f:
        content = f.read()
    return set(re.findall(r'\(no_inverse (\S+)\)', content))


def convert_file(input_path, output_path, append=False):
    """Batch convert all MkIndexed entries in a source file."""
    with open(input_path, 'r', encoding='utf-8') as f:
        source = f.read()

    existing_names = get_existing_names(output_path) if append else set()
    ...
    for form in forms:
        ...
        if not (isinstance(parsed, list) and parsed and parsed[0] == 'MkIndexed'):
            continue

        # Extract name to check for duplicates before full conversion
        inner = parsed[2]
        if not (isinstance(inner, list) and inner and inner[0] in ('MkAxiom', 'MkTheorem')):
            continue
        name = inner[1]
    ...
    print(f"Added: {len(converted)}  |  Skipped (already exist): {skipped}  |  Output: {output_path}")
...
def main():
    ...
    parser.add_argument('--append', action='store_true', help='Append new rules only, skip duplicates')
    args = parser.parse_args()

    convert_file(args.input, args.output, append=args.append)
